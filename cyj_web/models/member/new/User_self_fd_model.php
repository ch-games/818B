...
class User_self_fd_model extends MY_Model {
    ...
    public function user_self_fd_data(){
        ...
        $allbet = 0;
        $allbet = $fdata['fc']['bet'] + $fdata['sp']['bet'] + $fdata['fc']['six_bet'];
        foreach ($type as $k => $v) {
            //總打碼
            ...
            if ($v == 'mg') {
                ...
                $allbet += $vdata['agdz']['bet'] + $vdata['agter']['bet'];
            }elseif($v == 'gpi'){
                if(empty($vdata['gpidz']['bet'])){
                    $vdata['gpidz']['bet'] = 0;
                }
                $allbet += $vdata['gpidz']['bet'];
            }elseif($v == 'gd'){
                if(empty($vdata['gddz']['bet'])){
                    $vdata['gddz']['bet'] = 0;
                }
                $allbet += $vdata['gddz']['bet'];
            }
        }
        ...
        $userIds['six_bet'] = sprintf("%.2f",($fdata['fc']['six_bet']+0));
        ...
        foreach ($type as $k => $v) {
            ...
            if ($v == 'mg') {
                ...
                $userIds['total_e_fd'] += $userIds['agdz_fd'] + $userIds['agter_fd'];

            }elseif($v == 'gpi') {
                $userIds['gpidz_bet'] = sprintf("%.2f",($vdata['gpidz']['bet']+0));
                $userIds['gpidz_fd'] = sprintf("%.2f",$vdata['gpidz']['bet']*$re['gpidz_discount']*0.01);
                $userIds['total_e_fd'] += $userIds['gpidz_fd'];

            }elseif($v == 'gd') {
                $userIds['gddz_bet'] = sprintf("%.2f",($vdata['gddz']['bet']+0));
                $userIds['gddz_fd'] = sprintf("%.2f",$vdata['gddz']['bet']*$re['gddz_discount']*0.01);
                $userIds['total_e_fd'] += $userIds['gddz_fd'];
    ...
    public function user_videobet($arr =array(),$type){
        ...
        foreach ($type as $k => $v) {
            ...
            $maps['pkusername'] = $_SESSION['username'];
            $maps['site_id'] = SITEID;
            $maps["scene_endtime >="] = $arr['begin_date'];
            $maps["scene_endtime <="] = $arr['end_date'];
            ...
            switch ($k) {
                ...
                case 'ag':
                    ...
                    //捕魚
                    $this->video_db->from($k.'_cash_recordh');
                    $this->video_db->select("/* parallel */ sum(cost) as bet");
                    $this->video_db->where('platform_type','HUNTER');
                    //$this->video_db->where('data_type','HBR');
                    $video_arr['agter'] = $this->video_db->where($maps)->get()->row_array();
    ...
    public function user_self_fd_data_do(){
        ...
        $data_fdl['six_bet'] = $fdata['six_bet'] - $old_data['six_bet'];
        ...
        $data_fdl['six_fd'] = $fdata['six_fd'] - $old_data['six_fd'];
