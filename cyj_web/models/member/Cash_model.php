...
class Cash_model extends MY_Model
{
    ...
    public function get_bank_arr()
    {
        ...
        foreach ($bank_arr as $k => $v) {
            if ($v['id'] == '100' || $v['id'] == '101' || $v['id'] == '102') {
                array_push($arrkey, $v);
                unset($bank_arr[$k]);
            }
        }
        $bankData = array_merge_recursive($arrkey, $bank_arr);
        return $bankData;
    }
    ...
    {
        $this->db->from('k_bank');
        $this->db->select('id,card_ID,card_userName');
    ...
    public function get_bank_in_arr($level_id)
    {
        ...
        foreach ($banks as $k => $v) {
            if ($v['bank_type'] == '100' || $v['bank_type'] == '101' || $v['bank_type'] == '102') {
                array_push($bankshuf, $v);
                unset($banks[$k]);
            }
        }
        $banksData = array_merge_recursive($bankshuf, $banks);
        return $banksData;
    }


    public function get_setmoney()
    {   //線上存款，獲取存款文案和銀行信息
    ...
    public function add_cash_record($uid, $username, $credit, $money, $remark)
    { //插入現金交易記錄
        $data_c = array();
        $data_c['uid'] = $uid;
    ...
    public function out_cash_record()
    {      //獲取出款信息
    ...
    public function edit_pass($newpw)
    {     //修改密碼
        $this->db->from('k_user');
        $this->db->where('uid', $_SESSION['uid']);
        $this->db->where('site_id', SITEID);
    ...
    public function get_order_num($order_num)
    {    //查詢提交次數
        $this->db->from('k_user_bank_in_record');
        $this->db->select('order_num');
