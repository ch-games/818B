...
class Kjtpay_model extends Online_api_model {
	...
	function get_all_info($url,$order_num,$s_amount,$bank,$pay_id,$pay_key,$vircarddoin){
		...
		$ServerUrl = 'http://'.$url.'/index.php/pay/kjtpay_callback';//商戶後臺通知地址
		...
		$service = "create_instant_trade";//接口名稱
		$version = "1.0";//版本
		$partner_id = $pay_id;//商戶號
		$_input_charset = "UTF-8";
		$sign_type = "MD5";//加密方式
		$return_url = $ServerUrl;//回調地址
		$request_no = $order_num;//訂單號
		$trade_list=$request_no."~shopname~".$mo."~1~".$mo."~~".$vircarddoin."~1~instant~2~~~~~".date('YmdHis')."~".$ServerUrl;//交易列表
		$buyer_id = "anonymous"; //買家ID  固定值
		$buyer_id_type = 1; //買家ID類型 固定值
		$pay_method="online_bank^".$mo."^".$bank.",C,GC";//支付方式
		$go_cashier = "Y";//是否跳轉銀行收銀臺
