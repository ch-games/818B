...
class Bet_model extends MY_Model {
    ...
    public function get_ft_match($id,$type='',$dsorcg){
    ...
    public function get_ftp_match($id,$type='',$dsorcg){
        ...
        if($d && $dsorcg==1){
            foreach($d as $k =>$r){
                ...
                if($id ==$r[0] ){
                    ...
                    $zqgq[0]['Match_Time']=str_replace('<font style=background-color=red>半场</font>','半场',$r[1]);
                    $zqgq[0]['Match_Ho']=$r[9];
    ...
    public function get_bk_match($id,$type='',$dsorcg){
        if($dsorcg==2)$sql="SELECT Match_CoverDate,Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest, Match_RGG, Match_Name, Match_IsLose, Match_DxDpl, Match_DxXpl, Match_DxGG, Match_Ho, Match_Ao, Match_MasterID, Match_GuestID, Match_ShowType, Match_Type, Match_DsDpl, Match_DsSpl,'1' as lose_ok from lq_match where ScoreCheck=1 and Match_Id=$id AND Match_CoverDate >now()  limit 0,1";
        else $sql="SELECT Match_CoverDate,Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest, Match_RGG, Match_Name, Match_IsLose, Match_DxDpl, Match_DxXpl, Match_DxGG, Match_Ho, Match_Ao, Match_MasterID, Match_GuestID, Match_ShowType, Match_Type, Match_DsDpl, Match_DsSpl,'1' as lose_ok from lq_match where Match_Id=$id AND Match_CoverDate >now()  limit 0,1";
        $query=$this->public_db->query($sql);
        $d=$query->result_array();
        return $d;
    }
    public function get_bkp_match($id,$type='',$dsorcg){
        $this->load->driver('cache', array('adapter' => 'redis', 'backup' => 'file'));
    ...
    public function addbetds($d){
        ...
        $record['is_shiwan'] = $d['is_shiwan'];//判断是否试玩账号
        $record['index_id'] = $d['index_id'];//前台站点
        ...
        else
        {
            if($q1==1 && $q2>0 && $q3 >0) {
                ...
                $conn = Mongo_cli::getInstace(MGO_HOST,MGO_PORT,MGO_SP_USERNAME,MGO_SP_PASSWORD,MGO_SP_DBNAME);
                $tab = MGO_SP_DBNAME;
            
                if(!isset($conn->isok)){
                    try{
                       $res = $conn->$tab->insert($d);
                    }catch(Exception $e){
                        $data = serialize($e);
    ...
    //当场次限额查询
    public function checkxe2($dsorcg,$bet_money,$money,$uid,$matchid){
        if($dsorcg ==2){
            //闯关无法区分是足球还是篮球,也就是说会把所有过关注单和与限额比较
            $start = date('Y-m-d').' 0:0:0';
            $end = date('Y-m-d').' 23:59:59';
            $query = $this->private_db->query("select sum(bet_money) money from k_bet_cg_group where uid=$uid and bet_time>='$start' and  bet_time<='$end'");
            $row = $query->row_array();
            if(($row['money']+$bet_money) > $money){
                return $money-$row['money'];
            }else{
                return 0;
            }
        }else{
            $query = $this->private_db->query("select sum(bet_money) money from k_bet where uid=$uid and match_id ='$matchid'");
            $row = $query->row_array();
            if(($row['money']+$bet_money) > $money){
                return $money-$row['money'];
            }else{
