...
class Match_model extends MY_Model {
    ...
    public function FootballPlaying($r){
        ...
        $d=$this->cache->get('ZQGQ_JSON');
        $d=json_decode($d,true);
        $REDISKEY='SMManage_'.SITEID.'_FT_*';
        ...
        if($d){
            foreach($d as $k =>$r){
                ...
                $zqgq[$k]['Match_Time']=str_replace('background-color=','color:',$r[1]);
                $zqgq[$k]['Match_Ho']=$r[9];
    ...
    public function BasketballPlaying($r){
        ...
        $d=$this->cache->get('LQGQ_JSON');
        $d=json_decode($d,true);
        $REDISKEY='SMManage_'.SITEID.'_BK_*';
        $SMM=$this->cache->getkeys($REDISKEY);
        $Match_Name=array();
        $d=$d[1];
        $legname=[];
        if($d){
            ...
            foreach($d as $k =>$r){
                $dx    =    array();
                //if($r['legname'])
                if(!in_array($r[2], $legname))$legname[]=$r[2];
                if( strpos($r[2], '测试')===false && strpos($r[5], '第4节')===false){
                    $zqgq[$k]['Match_ID']=$r[0];
                    $zqgq[$k]['Match_Master']=$r[5];
                    $zqgq[$k]['Match_Guest']=$r[6];
                    $zqgq[$k]['Match_Name']=$r[2];
    ...
    public function FootballToday($r){
        ...
        $Match_Type="Match_Type IN(1)";
        if(strpos($docroot,'A_admin/P_admin')!==false) {
            $limit='';
            $date=date('Y-m-d H:i:s',(time()-3600*3));
            $Match_Type="Match_Type IN(1,2)";
        }
        $where='';
        $legar=array();
        if($r['leg']) {
        ...
        $where=" where $Match_Type and Match_CoverDate between '".$date."' and '".date('Y-m-d 23:59:59')."' $where";
    ...
    public function FootballGG($r){
        ...
        $sql = "SELECT Match_ID, Match_HalfId, Match_Date, Match_Time, Match_Master, Match_Guest,Match_Bmdy,Match_BHo, Match_RGG, Match_Name, Match_IsLose,Match_Bdpl, Match_BzM, Match_BzG, Match_BzH, Match_DxDpl,Match_Bgdy, Match_BAo, Match_Bxpl, Match_Bhdy, Match_BRpk, Match_Hr_ShowType, Match_DxXpl,Match_Bdxpk, Match_DxGG, Match_Ho, Match_Ao, Match_MasterID, Match_GuestID, Match_ShowType, Match_Type, Match_DsDpl, Match_DsSpl,Match_IsShowb
        FROM bet_match Where ScoreCheck=1 and  Match_BzM>0 and  Match_BzG>0 and  Match_BzH>0 and match_name not like '%特别投注%' and Match_CoverDate between '".date('Y-m-d H:i:s')."' and  '".date('Y-m-d 23:59:59')."'  $where order by Match_CoverDate ,iPage,match_name,Match_Master,Match_ID,iSn  $limit";
        $query=$this->public_db->query($sql);
        $d['d']=$query->result_array();
        //获取总联赛名
        $sql = "SELECT Match_Name FROM bet_match where ScoreCheck=1 and  Match_BzM>0 and  Match_BzG>0 and  Match_BzH>0 and match_name not like '%特别投注%' and Match_CoverDate between '".date('Y-m-d H:i:s')."' and  '".date('Y-m-d 23:59:59')."' $where ";
        $query=$this->public_db->query($sql);
        $dMatch_Name=$query->result_array();
        $Match_Name=array();
        foreach ($dMatch_Name as $key => $value) {
    ...
    public function BasketballToday($r){
        ...
        FROM lq_match where  $Match_Type and Match_CoverDate between '".$date."' and '".date('Y-m-d 23:59:59')."' $where  order by Match_CoverDate ,iPage,iSn,Match_ID,match_name,Match_Master $limit";
        $query=$this->public_db->query($sql);
        $d['d']=$query->result_array();
        //获取总联赛名
    ...
    public function BasketballGG($r){
        ...
        $limit=" limit $limit,$pnum";
        $sql = "SELECT Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest, '' as Match_BzM, '' as Match_BzG, '' as Match_BzH ,Match_RGG, Match_Name, Match_IsLose, Match_DxDpl, Match_DxXpl, Match_DxGG, Match_Ho, Match_Ao, Match_MasterID, Match_GuestID, Match_ShowType, Match_Type, Match_DsDpl, Match_DsSpl
        FROM lq_match where ScoreCheck=1 and Match_CoverDate > '".date('Y-m-d H:i:s')."'   order by Match_CoverDate ,iPage,iSn,Match_ID,match_name,Match_Master $limit";
        $query=$this->public_db->query($sql);
        $d['d']=$query->result_array();
        $sql = "SELECT count(*) as nums FROM  lq_match   where  ScoreCheck=1 and Match_CoverDate > '".date('Y-m-d H:i:s')."' ";
        $query=$this->public_db->query($sql);
    ...
    public function FootballMGG($r){
        ...
        $sql = "SELECT Match_ID, Match_HalfId, Match_Date, Match_Time, Match_Master, Match_Guest,Match_Bmdy,Match_BHo, Match_RGG, Match_Name, Match_IsLose,Match_Bdpl, Match_BzM, Match_BzG, Match_BzH, Match_DxDpl,Match_Bgdy, Match_BAo, Match_Bxpl, Match_Bhdy, Match_BRpk, Match_Hr_ShowType, Match_DxXpl,Match_Bdxpk, Match_DxGG, Match_Ho, Match_Ao, Match_MasterID, Match_GuestID, Match_ShowType, Match_Type, Match_DsDpl, Match_DsSpl,Match_IsShowb
        FROM bet_match Where  ScoreCheck=1 and  match_name not like '%特别投注%' and Match_BzM>0 and  Match_BzG>0 and  Match_BzH>0 and Match_CoverDate > '".date('Y-m-d 23:59:59')."'  $where order by Match_CoverDate ,iPage,match_name,Match_Master,Match_ID,iSn  $limit";
        $query=$this->public_db->query($sql);
        $d['d']=$query->result_array();
        //获取总联赛名
        $sql = "SELECT Match_Name FROM bet_match where ScoreCheck=1 and match_name not like '%特别投注%' and Match_BzM>0 and  Match_BzG>0 and  Match_BzH>0 and Match_CoverDate > '".date('Y-m-d 23:59:59')."' $where ";
        $query=$this->public_db->query($sql);
        $dMatch_Name=$query->result_array();
        $Match_Name=array();
        foreach ($dMatch_Name as $key => $value) {
    ...
    public function BasketballMGG($r){
        ...
        $limit=" limit $limit,$pnum";
        $sql = "SELECT Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest, '' as Match_BzM, '' as Match_BzG, '' as Match_BzH ,Match_RGG, Match_Name, Match_IsLose, Match_DxDpl, Match_DxXpl, Match_DxGG, Match_Ho, Match_Ao, Match_MasterID, Match_GuestID, Match_ShowType, Match_Type, Match_DsDpl, Match_DsSpl
        FROM lq_match where ScoreCheck=1 and  Match_Type=0 and  Match_CoverDate > '".date('Y-m-d 23:59:59')."'   order by Match_CoverDate ,iPage,iSn,Match_ID,match_name,Match_Master $limit";
        $query=$this->public_db->query($sql);
        $d['d']=$query->result_array();
        $sql = "SELECT count(*) as nums FROM  lq_match   where ScoreCheck=1 and  Match_Type=0 and Match_CoverDate > '".date('Y-m-d 23:59:59')."' ";
        $query=$this->public_db->query($sql);
        $d['nums']=$query->row_array();
        $d['nums']=$d['nums']['nums'];
        $d['page']=ceil($d['nums']/$pnum);
