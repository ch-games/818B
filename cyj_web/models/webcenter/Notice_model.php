...
class Notice_model extends MY_Model {
	...
	public function getNotice($type){
		...
		$map1 = "(sid = '0' or find_in_set('".SITEID."',sid)) and notice_state = '1' and notice_cate != '6' and (notice_type in (".$v_c.") or notice_type is null)";
		...
		if ($type == '8') {
			...
			$notice3 = $this->public_db->where("(sid = '0' or find_in_set('".SITEID."',sid)) and notice_cate='s_p' and notice_date > '".$starttime."' and notice_date < '".$endtime."'")->order_by("notice_date DESC")->get('site_notice')->result_array();
		...
		if(!empty($notice)){
			$d['left'] = $this->return_notice($notice,1);
			$d['up'] = $this->return_notice($notice,2);
			return $d;
		}
	}
    ...
    function get_user_nnews(){
		...
		//获取未读消息数量 uid site_id index_id is_delete islook msg_time  
		$list = $this->db->from('k_user_msg')->select('msg_id')->where("(uid='" . $_SESSION["uid"] . "' or (uid = '' and (level_id like '%" . $_SESSION['level_id'] . "%' or level_id = '-1'))) and site_id ='".SITEID."' and index_id = '".INDEX_ID."'  and is_delete = '0' and islook='0' and msg_time > '".$start_date."'")->get()->num_rows();
		$count = !empty($list)?$list:0;
        return $count;
    }
