        ...
        if(!empty($float) && is_array($float)){
	        ...
	        foreach ($floatall as $k => $val) {
	        	...
	        	if($val['fid'] == $typel){
					...
					if($val['is_blank'] == 1 && $val['urltype'] != 20){
						$data['floatl'][$sort]['b_str'] = 'target="_blank"';
					}
					if($val['is_inter'] == 0){
						$data['floatl'][$sort]['click'] = $this->geturl($val['urltype'],$val['url'],$val['lottery_type'],$val['dz']);
						$data['floatl'][$sort]['url'] = "javascript:;";
					}else{
					...
					if($val['is_inter'] == 0){
						$data['floatr'][$sort]['click'] = $this->geturl($val['urltype'],$val['url'],$val['lottery_type'],$val['dz']);
						$data['floatr'][$sort]['url'] = "javascript:;";
					}else{
						$data['floatr'][$sort]['click'] = '';
					}
        		}
	        }
	        if(!empty($data['floatl'])){
	        	$data['floatl'] = array_values($data['floatl']);
	        }
	        if(!empty($data['floatr'])){
			...
			if($closel > 0){
				...
				$data['floatl'][$closel-1]['url'] = "javascript:;";
			...
			$data['floatr'][$closer-1]['url'] = "javascript:;";
