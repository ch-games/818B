...
class Register_web_model extends MY_Model {
	...
	function user_reg($arr,$reg_state){
		...
		$this->db->select("zhuce_msg,zhuce_msg2");
		...
		if ($arr['money'] > 0) {
			...
			if($date[0]['zhuce_msg'] =="" && $date[0]['zhuce_msg2'] == ""){
				$dataM['msg_info']  = $arr['username'] . ',' . "會員注册贈送優惠" . $reg_state['money'] . "元 祝您游戏愉快！";
			}else{
				$dataM['msg_info']  = $arr['username'] . ',' . $date[0]['zhuce_msg'] . $reg_state['money'] . $date[0]['zhuce_msg2'];
			}
    ...
    //试玩信息插入表格
    public function add_shiwan($username,$password,$agent_demo,$shiwan_money){
        $this->load->model('sports/User_model','User_model');
    	$data_s['username'] = $username;
		$data_s['password'] = $password;
		$data_s['money'] = $shiwan_money; //试玩赠送金额
        ...
        $_SESSION["token"]=$this->User_model->redis_update_token([$shi_S,$username,$data_s['agent_id'],0,1,SITEID,INDEX_ID,$data_s['ua_id'],$data_s['sh_id']]);
		return $shi_S;
    }
	
	//获取试玩配置
	public function get_shiwan_views(){
		$map = array();
		$map['table'] = "k_user_reg_config";
		$map['where']['site_id'] = SITEID;
		$map['where']['index_id'] = INDEX_ID;
		$map['select'] = 'shiwan_money,is_shiwan';
		$shiwan = $this->rfind($map);
		return $shiwan;
	}
