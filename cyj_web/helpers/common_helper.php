...
//广东11 赔率排序
function pailie_odds_gd11($arr){//p($arr);exit;
	foreach ($arr as $k => $v){
		//1-5球 数字球 排列数组
		if(is_numeric($v['input_name'])){
			switch ($v['fc_type']){
				case '第一球' : $array[0][0][] = $v;break;
				case '第二球' : $array[0][1][] = $v;break;
				case '第三球' : $array[0][2][] = $v;break;
				case '第四球' : $array[0][3][] = $v;break;
				case '第五球' : $array[0][4][] = $v;break;
			}
		}else{	//非数字球时
			switch ($v['fc_type']){
				case '总和' : $array1[0][] = $v;break;
				case '第一球' : $array1[1][0][0][] = $v;break;
				case '第二球' : $array1[1][0][1][] = $v;break;
				case '第三球' : $array1[1][0][2][] = $v;break;
				case '第四球' : $array1[1][0][3][] = $v;break;
				case '第五球' : $array1[1][0][4][] = $v;break;
				case '任选' : $array2[] = $v;break;
				case '组选' : $array3[] = $v;break;
				case '直选' : $array4[] = $v;break;
			}
		}
	}
	ksort($array1);
	$data[0] = $array1;	//两面盘数组（总和、1-5球的非数字球）
	$data[1] = $array; //1-5球的数字球
	$data[2] = $array2; //任选玩法
	$data[3] = $array3; //组选玩法
	$data[4] = $array4; //直选玩法
	return $data;
...
function func_getdid(){
    return  date("YmdHis") .substr(microtime(),4,4). mt_rand("10", "99");
}
...
// 计算福彩3D和排列3的期数
function func_fc_qishu($kaijiang)
{
    ////过年  假期七天
    $lunar=new Lunar();
    $qishu = $lunar->getDaysBetweenSolar(date('Y',strtotime('+12 hours')),date('m',strtotime('+12 hours')),date('d',strtotime('+12 hours')),1,0)-7;
    if(($kaijiang) < date('H:i:s',strtotime('+12 hours')))
        $qishu++;
...
function tamaodds($odds){
  if(!empty($odds)){
    ...
    $odds1['suzi'] = $arr1;
    $odds1['hanzi'] = sortbytama($arr2);
    if(empty($odds1['hanzi']))
        unset($odds1['hanzi']);
    return $odds1;
  }
}

function sortbytama($arr){
    $sort = ['1-10','11-20','21-30','31-40','41-49','大','合大','大单','尾大','红波','小','合小','小单','尾小','绿波','单','合单','大双','家禽','蓝波','双','合双','小双','野兽'];
    $return = [];
    foreach ($sort as $v){
        if(isset($arr[$v]))
            $return[] = $arr[$v];
