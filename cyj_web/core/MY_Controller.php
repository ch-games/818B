...
class MY_Controller extends CI_Controller {
	...
	public function __construct() {
        ...
        $this->Maintain_model->maintain_check('pc',SITEID,INDEX_ID);
		...
		switch ($whuri_1) {
            ...
            case 'lottery':
                ...
                if(($whuri_3=='bet' || $whuri_3=='index' || $whuri_3=='liuhecaijson') && ($payload['lotteryPan']>=222 && $payload['lotteryPan']<=234)){
                  ...
                  $wgtype=4;
                  $cate_type='jl_k3';
				}elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='gx_k3'){
                  $wgtype=4;
                  $cate_type='gx_k3';
				}elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='ah_k3'){
                  $wgtype=4;
                  $cate_type='ah_k3';
				}elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='gd_11'){
                  $wgtype=4;
                  $cate_type='gd_11';
				}elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='sd_11'){
                  $wgtype=4;
                  $cate_type='sd_11';
				}elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='jx_11'){
                  $wgtype=4;
                  $cate_type='jx_11';
				}elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='xy_28'){
                  $wgtype=4;
                  $cate_type='xy_28';
                }elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='bj_28'){
                  $wgtype=4;
                  $cate_type='bj_28';
                }elseif(($whuri_3=='bet' || $whuri_3=='get_json' ) && $payload['lotteryId']=='pc_28'){
                  $wgtype=4;
                  $cate_type='pc_28';
                }else{
                  $wgtype=1;
			...
			if(count($tmp_host)>2){
				if(strpos($tmp_host[0],'pay') !== false){
					...
					if(!empty($key['wap_url'])){
						$this->db->from('k_user_agent_domain');
						$this->db->where('domain',$_SERVER['HTTP_HOST']);
						$this->db->where('site_id',$siteid);
						$this->db->where('index_id',$indexid);
						$this->db->select('intr');
						$this->db->where('state',1);
						$keys = $this->db->get()->row_array();
						if($keys['intr']){
							$intr = $keys['intr'];
						}else{
