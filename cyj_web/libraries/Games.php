...
class Games {
	...
	private $aburl = AB_API;
	private $pturl = PT_API;
	private $otherurl = API;
	/*
	...
	*/
	function forwardGame($loginname, $gtype, $gametype, $limit = "", $limitype = "", $lang = "CN", $cur = 'RMB', $sw = '0',$ismobile = '0') {
		$crypt = $this->DES1($this->DESKey);//获取为站点信息表中的 site_des_key,site_md5_key
		...
		if ($gtype == "og") {
			...
			$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/cur=" . $cur ."/\\\\/ismobile=" . $ismobile .
				"/\\\\/gametype=" . $gametype . "/\\\\/limit=" . $limit . "/\\\\/lang=" . $lang . "/\\\\/gtype=" . $gtype. "/\\\\/ip=" . $ip. "/\\\\/ismobile=" . $ismobile. "/\\\\/sw=" . $sw);
		}
    ...
    function AbEditAccountPwd($loginname, $pwd) {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/password=" . $pwd);
		$Key = $this->getKey($params);

		$url = $this->_getUrl("ab") . "/editaccountpwd?params=" . $params . "&key=" . $Key;
		//echo $url;echo "<br>";echo $this->siteid;
		$result = $this->web_curl($url);
		return $result;
	}
	...
	function CreateAccount($loginname, $agent_id, $gtype, $index_id = "a", $cur = 'RMB', $sw = 0) {
		...
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/agent_id=" . $agent_id . "/\\\\/index_id=" . $index_id . "/\\\\/cur=" . $cur . "/\\\\/sw=" . $sw . "/\\\\/gtype=" . $gtype);
		$Key = $this->getKey($params);
	...
	*/
	...
	function GetBalance($loginname, $gtype) {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/gtype=" . $gtype);
	...
	function GetAllBalance($loginname,$types='') {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname. "/\\\\/types=" . $types);
		$Key = $this->getKey($params);
	...
	function TransferCredit($loginname, $gtype, $type, $credit) {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/type=" . $type . "/\\\\/credit=" . $credit . "/\\\\/gtype=" . $gtype);
	...
	function GetBetRecord($gtype, $loginname, $orderId, $videotype, $gametype, $s_time, $e_time, $agentid, $page = 1, $page_num = 15) {
		...
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname
			...
			. "/\\\\/e_time=" . $e_time . "/\\\\/agentid=" . $agentid . "/\\\\/page=" . $page . "/\\\\/page_num=" . $page_num. "/\\\\/gtype=" . $gtype);
	...
	function GetAvailableAmountByUser($gtype, $username, $s_time = "", $e_time = "", $dz = '') {
		...
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $username . "/\\\\/s_time=" . $s_time
			. "/\\\\/e_time=" . $e_time . "/\\\\/dz=" . $dz. "/\\\\/gtype=" . $gtype);
	...
	function GetAvailableAmountBySiteid($gtype, $s_time, $e_time, $dz = '') {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/s_time=" . $s_time . "/\\\\/e_time=" . $e_time . "/\\\\/dz=" . $dz. "/\\\\/gtype=" . $gtype);
	...
	function GetAvailableAmountByAgentid($gtype, $agentid, $s_time, $e_time, $dz = '') {
		...
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/agentid=" . $agentid
			. "/\\\\/s_time=" . $s_time . "/\\\\/e_time=" . $e_time . "/\\\\/dz=" . $dz. "/\\\\/gtype=" . $gtype);
	...
	function GetUserAvailableAmountByUser($gtype, $username, $s_time = "", $e_time = "", $dz = '', $notbk = '') {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $username . "/\\\\/s_time=" . $s_time
			. "/\\\\/e_time=" . $e_time . "/\\\\/dz=" . $dz . "/\\\\/notbk=" . $notbk. "/\\\\/gtype=" . $gtype);
	...
	function GetUserAllAvailableAmountByAgentid($gtypes, $agentid, $s_time, $e_time) {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/agentid=" . $agentid .
			"/\\\\/types=" . $gtypes . "/\\\\/s_time=" . $s_time . "/\\\\/e_time=" . $e_time . "/\\\\/dz=" . $dz. "/\\\\/gtype=" . $gtype);
	...
	function GetUserAllAvailableAmount($gtypes, $username, $s_time, $e_time, $notbk = '') {
		...
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $username
			. "/\\\\/s_time=" . $s_time . "/\\\\/types=" . $gtypes . "/\\\\/e_time=" . $e_time . "/\\\\/notbk=" . $notbk. "/\\\\/gtype=" . $gtype);
	...
	*/
	...
	function encrypt($input) {
		...
		return preg_replace("/\s*/", '', $data);//正则，去掉空格
	}
