...
class News extends MY_Controller {
	...
	public function sms(){
		...
		if($id){
			...
			$look_log = $this->News_model->get_look_log(array('uid'=>$_SESSION['uid']));
	...
	public function sms_change(){
		...
		if($type =='look'){
			...
			$query = $this->News_model->add_look_log($map);
			if($query){
				echo  1;exit;
			}else{
				echo  2;exit;
	...
	public function sms_del(){
		...
		$query = $this->News_model->del_sms($map);
		if($query){
			echo  1;exit;
		}else{
			echo  2;exit;
