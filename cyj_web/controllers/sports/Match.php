...
class Match extends MY_Controller {
    ...
    public function Footballtoday(){
        ...
        foreach($d['d'] as $i=>$rows){
            // $SMM_CHECK=false;
            if($SMM){
                $rows=$this->sportsbet->SMM($SMM,$rows,'FT',$sprotsorderpktype);
                //print_r($rows);
                if($rows==null) {
                    $SMM_CHECK++;
                    continue;
    ...
    public function BasketballPlaying(){
        ...
        if($d['d']){
            foreach($d['d'] as $i=>$rows){
                if($SMM){
                    $rows=$this->sportsbet->SMM($SMM,$rows,'BK',$sprotsorderpktype);
                    //print_r($rows);
                    if($rows==null) {
                        $SMM_CHECK++;
                        continue;
    ...
    public function BKRresults(){
        ...
        foreach($d as $i=>$rows){
            ...
            $json["db"][$i]["TG_Inball_OK"]      =    $rows["TG_Inball_OK"]  >= 0 ? $rows["TG_Inball_OK"]: '无效';
            $json["db"][$i]["MB_Inball_OK"]      =    $rows["MB_Inball_OK"]  >= 0 ? $rows["MB_Inball_OK"]: '无效';
