...
class Bet extends MY_Controller {
    ...
    public function makebetshow(){
        ...
        $_SESSION['SportMakeBetShowMatchId']=$post['matchid'];
        if($post['sport_type']=='BKP'){//投注安全机制
            // usleep(500000);
        }
        $d=$this->makebet($post);
    ...
    public function makebet($post){
        ...
        $d['msg']      =    '';
        $ert=$this->User_model->RefreshRate(1,$this->user_data);
        if($ert && $post['sport_type']=='BKP'){//投注安全机制
            //$this->gojson($echo,'msg','刷新太快[' .date('Y-m-d H:i:s',$ert).']<br>之后再接受下注');
            $this->gojson($echo,'msg','您刷新太快了！');
        }
        ...
        if($matchid){
            ...
            if($pkjc==true){
                if($post['dsorcg']==2){
                    /*闯关限额*/
                    $setdata=$this->sportsbet->checkxe($setdata_=$this->Bet_model->checkxe($this->user_data['uid'],$this->user_data['agentid']),$sport_type,'zhgg');
                }else{
                    /*单式限额*/
                ...
                //                                 
                if($post['bet_money'] && $post['bet_money']<$setdata[2]){
                    $this->gojson($echo,'msg','下注金额不能低于限额'.$setdata[2]);
                }elseif($post['bet_money'] &&  $post['bet_money']>$setdata[1]){
                    $this->gojson($echo,'msg','下注金额不能高于限额'.$setdata[1]);
                }elseif($post['bet_money'] &&  $cando=$this->Bet_model->checkxe2($post['dsorcg'],$post['bet_money'],$setdata[0],$this->user_data['uid'],$matchid)){
                    $this->gojson($echo,'msg','该类别下注总金额不能高于限额'.$setdata[0].'!'.$sport_type.'--'.$pk_array['pkinfo'][0].'此类别还可下注'.$cando);
                }

                if($sport_type=='FT')       $data=$this->Bet_model->get_ft_match($matchid,$pk_array['pkinfo'][0],$post['dsorcg']);
                elseif($sport_type=='FTP')  $data=$this->Bet_model->get_ftp_match($matchid,$pk_array['pkinfo'][0],$post['dsorcg']);
                elseif($sport_type=='BK')   $data=$this->Bet_model->get_bk_match($matchid,'',$post['dsorcg']);
                elseif($sport_type=='BKP')  $data=$this->Bet_model->get_bkp_match($matchid,'',$post['dsorcg']);
                ...
                $REDISKEY='SMManage_'.SITEID.'_'.str_replace('P','',$sport_type).'_*';
                $SMM=$this->cache->getkeys($REDISKEY);
                // echo str_replace('P','',$sport_type);
                $this->load->library('sportsbet'); //调用体育公共类库 进行赔率算法计算
                if($SMM) $data[0]=$this->sportsbet->SMM($SMM,$data[0],str_replace('P','',$sport_type),$sprotsorderpktype);
                /*关闭赛事判断 */
                if($data){
                    if (strpos('TEST'.$data[0]['Match_Name'],'测试')>0)  $this->gojson($echo,'msg','测试赛事禁止下注！');
                    ...
                    if($pk_array['pkinfo'][0]=='rq'){
                        ...
                        else  $this->gojson($echo,'msg','01主客让分信息错误');
                        //异常赔率 极限赔率判断
                        if($sport_type=='BK' || $sport_type=='BKP' ){//篮球
                            if($data[0]['Match_Ho']>=1.5 || $data[0]['Match_Ao']>=1.5) $this->gojson($echo,'msg','02下注失败');
                        }

                        if($sport_type=='FTP' || $sport_type=='FT' ){//足球

                            if($data[0]['Match_Ho']>=3 || $data[0]['Match_Ao']>=3 ||$data[0]['Match_Ho']<0.02 || $data[0]['Match_Ao']<0.02) $this->gojson($echo,'msg','03下注失败');
                        }
                        ...
                        //异常赔率 极限赔率判断
                        if($sport_type=='BK' || $sport_type=='BKP' ){//篮球
                            if($data[0]['Match_DxDpl']>=1.5 || $data[0]['Match_DxXpl']>=1.5 ||$data[0]['Match_DxDpl']<0.02 || $data[0]['Match_DxXpl']<0.02) $this->gojson($echo,'msg','04下注失败');
                        }
                        if($sport_type=='FTP' || $sport_type=='FT' ){//足球
                            if(
                                $data[0]['Match_DxDpl']>=3 || $data[0]['Match_DxXpl']>=3 ||$data[0]['Match_DxDpl']<0.02 || $data[0]['Match_DxXpl']<0.02
                            ) $this->gojson($echo,'msg','下注失败');
                        }
                        /*下大下小判断*/
                        if($pk_array['pkinfo'][1]==2) $Match_ShowType='Match_DxGG';
                        else if($pk_array['pkinfo'][1]==1) $Match_ShowType='Match_Bdxpk';
                        else  $this->gojson($echo,'msg','05大小盘信息错误');
                        //让分盘判断
                        $match_dxgg=$data[0][$Match_ShowType];
                    }
                    if ($ior[$team]==0 || $ior[$team]=='') $this->gojson($echo,'msg','06盘口已经关闭');
                    $d['data']['pl']     =$ior[$team];
    ...
    public function post_bet(){
        ...
        if($dsorcg==1){//单式
            //异常流程下注判断
            if($_SESSION['SportMakeBetShowMatchId']!=$pd['Match_ID'][0] || $_SESSION['SportMakeBetShowMatchId']=='') $this->gojson($echo,'msg','参数错误010');

            //加入延时 规避打水
            if($pd['Sport_Type'][0]=='BKP') sleep(3);
            else if($pd['Sport_Type'][0]=='FTP') sleep(2);

            //异常流程判断结束
            //$makebet (['matchid','sport_type','pk','oddpk','dsorcg']);
            $bet=$this->makebet(['matchid'=>$pd['Match_ID'][0],'sport_type'=>$pd['Sport_Type'][0],'pk'=>$pd['Match_Type'][0],'oddpk'=>$pd['Odd_PK'][0],'dsorcg'=>1,'bet_money'=>$pd['bet_money']]);
            ...
            if($cd['status']==1){
                $_SESSION['SportMakeBetShowMatchId']='';
            ...
            //限额放foreach里面太浪费资源了，单独拿出了判断一下即可。
            $this->makebet(['matchid'=>$pd['Match_ID'][0],'sport_type'=>$pd['Sport_Type'][0],'pk'=>$pd['Match_Type'][0],'oddpk'=>$pd['Odd_PK'][0],'dsorcg'=>2,'bet_money'=>$pd['bet_money']]);
            ...
            foreach($pd['Match_ID'] as $k=>$v){
                ...
                //篮球、足球不能混下
                if($pd['Sport_Type'][$k]!=$pd['Sport_Type'][0]) $this->gojson($echo,'msg','不同的类别不能进行赛事投注');
    ...
    public function check_point($bet,$pd){
        ...
        if($sprotsorderpktype[0]=='rq' || $sprotsorderpktype[0]=='dx'){
            ...
            if($sprotsorderpktype[0]=='rq'){
                ...
                //让分盘判断
                if($pd['Match_ShowType']!=$data[$Match_ShowType]) $this->gojson($echo,'msg','03盘口数据已变['.$data[$rqdxpk].']');
                //滚球 特殊情况过滤 当某一队占优势时 又出现对方让球
                /*
                if($data['lose_ok']!=1){
                   $score=explode(':',$data['Match_NowScore']);
                   if($data[$Match_ShowType]=='H' && $score[1]>$score[0]){ //当客队进球数大于主队时 盘口出现主让 禁止下注
                      $this->gojson($echo,'msg','09网络错误，请重新进行下注');
                   }
                   if($data[$Match_ShowType]=='C' && $score[0]>$score[1]){  
                      $this->gojson($echo,'msg','10网络错误，请重新进行下注');
                   }
    ...
    public function addbetds($bet,$pd,$userdata){
        ...
        /* match_time,*/       $d['match_time']     =  str_replace('<font style=background-color=red>半场</font>','半场',$data['Match_Time']);//比赛时间
