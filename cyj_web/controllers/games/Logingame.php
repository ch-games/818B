...
class Logingame extends MY_Controller
{
	...
	public function __construct()
	{
  	...
  	public function index(){
		...
		$data_info = $this->Games_model->GetGameInfo($gameid,$g_type);
		if(!empty($data_info)){
			if($data_info['status'] == 0 || $data_info['is_zs'] == 2){
				echo "<script>alert('敬请期待开放！')</script>";exit;
			}
		}else{
			echo "<script>alert('未知游戏！')</script>";exit;
		}
		$Games = new Games;
		$lang = "CN";
		$cur = "RMB";
	...
	public function PtDemo(){
		...
		$data_info = $this->Games_model->GetGameInfo($gameid,$g_type);
		if(!empty($data_info)){
			if($data_info['status'] == 0 || $data_info['is_sw'] == 2){
				echo "<script>alert('敬请期待开放！')</script>";exit;
			}
		}else{
			echo "<script>alert('未知游戏！')</script>";exit;
	...
	public function EgDemo(){
		$gameid = $this->input->get('gameid');
		if(empty($gameid)){
			echo "<script>alert('未知游戏！')</script>";exit;
		}
		$g_type = "eg";
		$data = $this->Games_model->GetGameInfo($gameid,$g_type);
		if(!empty($data)){
			if($data['status'] == 0 || $data['is_sw'] == 2){
				echo "<script>alert('敬请期待开放！')</script>";exit;
			}
		}else{
			echo "<script>alert('未知游戏！')</script>";exit;
		}
		echo "<script>alert('您进入的是EG试玩线路！')</script>";
		$url = 'http://demo.earlgame.com/testLogin?gameId='.$gameid;
		echo $this->echo_frame($url, $title);
	}
