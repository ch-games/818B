...
class Register_web extends MY_Controller {
	...
	{
		//获取试玩配置
		$shiwan = $this->Register_web_model->get_shiwan_views();
		//p($shiwan);die;
		if (empty($shiwan['is_shiwan'])) {
			echo "<script>alert('系统禁用了用户试玩功能,请联系管理员!');parent.location.href='/';</script>";exit;
		}
	...
	public function join_shiwan_do(){
		...
		$shiwan_money = $shiwan['shiwan_money'];
		...
		$shi_S = $this->Register_web_model->add_shiwan($username,$password,$agent_demo,$shiwan_money);
    ...
    public function join_member(){
		    ...
		    $is_life = $result['is_life']; //出生日期是否显示
			...
			$ads=$this->add("result", $result);
		     //是否显示代理人,验证码,和出生日期
			$this->add('is_code',$is_code);
			$this->add('is_life',$is_life);
			$this->add("pk_token", $pk_token);
	...
	public function join_member_do(){
		$zcname = strtolower($this->input->post('zcname'));//账号  strtolower  大写转小写
		...
		$result = $this->Register_web_model->rfind($map);
		unset($map);
		$sta_email    = explode('-', $result['email'])[1];
		$sta_passport = explode('-', $result['passport'])[1];
		$sta_mobile   = $result['mobile'];
		$sta_qq       = explode('-', $result['qq'])[1];
		$pk_token     = $this->input->post('pk_token');
		$this->tokenCk_form($pk_token);   //验证token，防止恶意注入
	    //表单验证
    	...
    	if (!empty($is_name)) {
			...
			if (empty($is_name['is_qq']) && !empty($qq)) {
				...
				// if (!preg_match("/^[a-zA-Z0-9]{4,11}$/", $qq)) {  //账号格式不正确
    //             	message('邮箱格式错误！');
    //         	}
			}
			//手机是否重复
		...
		if(!empty($sta_passport)){	// 判断身份证是否必填
			if (!empty($passport)) {
				if (!preg_match("/^(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/", $passport)) {
					message('您输入身份证号格式不正确，请重新输入身份证号!');
					exit;
				}
			}
		}
		...
		//手机号，需要切必填
		if($sta_mobile=='0-1'){ 
			if($result['ofcp']==1){ //如果限制为中国手机号
				if (!preg_match("/^[1][3578]\d{9}$/", $mobile)) {
					message('请输入中国境内手机号！');
					exit;
				}
			}else{
				if (!preg_match("/^[+]?[0-9]{6,13}$/", $mobile)) {
					message('请输入+开头的国际号码,或6至13个正确手机号！');
					exit;
				}
			}
		}
	...
	public function get_agent($type) {
		...
		if ($type == '3') {
			...
			if(empty($_SESSION['intr'])&&empty($_COOKIE['intr'])){
			...
			if (isset($user_reg_data) && $user_reg_data[1] != '') {
				if ($user_reg_data[0] == 0) {
					...
					if ($k == 'email') {
						...
						if($arr['ofcp']==1){
							$list[$k]['class'] = 'chenckChinaMobile'; //仅可使用中国手机号
						}
					}
